// Runtime theming. The 5 brand colours below drive every UI surface (dashboard,
// garage, impound) via CSS custom properties. Tailwind tokens reference these
// vars (see tailwind.config.ts), so setting them on :root recolours everything
// live. Derived shades (panel-raised, accent-dim, text-muted) are computed here
// rather than exposed, to keep the editable set small and always consistent.
...
// amt > 0 shifts toward white (lighten), amt < 0 toward black (darken).
...
// Coerces a possibly-partial/invalid theme into a full, valid Theme.
...
// Applies the theme to :root. Safe to call repeatedly (live preview).
