...
export default {
  ...
  theme: {
    extend: {
      // Colours are driven by CSS custom properties (see web/src/lib/theme.ts and
      // the :root defaults in src/styles/index.css) so the admin dashboard can
      // recolour every surface live. The `<alpha-value>` placeholder keeps the
      // Tailwind opacity modifiers (e.g. bg-accent/15) working with var() colours.
      colors: {
        bg: 'rgb(var(--c-bg-rgb) / <alpha-value>)',
        panel: 'rgb(var(--c-panel-rgb) / <alpha-value>)',
        'panel-raised': 'rgb(var(--c-panel-raised-rgb) / <alpha-value>)',
        // Structural tints track the primary text colour so borders stay visible
      ...
      fontFamily: {
        // Fonts adopted from the doors_creator reference UI.
        // Title & headings: Work Sans. Everything else: Open Sans.
